...
public class ThreatApplicationRule extends AbstractCapellaElementRule {
  ...
  public ThreatApplicationRule() {
    registerUpdatedReference(CybersecurityPackage.Literals.THREAT_APPLICATION__ASSET);
  }
  ...
  protected void premicesRelated(EObject element, ArrayList<IPremise> needed) {
    ...
    needed.addAll(createDefaultPrecedencePremices(element, CybersecurityPackage.Literals.THREAT_APPLICATION__ASSET));
  }
  ...
  protected void attachRelated(EObject element, EObject result, IContext context) {
    ...
    AttachmentHelper.getInstance(context).attachTracedElements(element, result,
        CybersecurityPackage.Literals.THREAT_APPLICATION__ASSET, context);
  }
  
  @Override
  protected void retrieveGoDeep(EObject source, List<EObject> result, IContext context) {
    super.retrieveGoDeep(source, result, context);
    result.add(((ThreatApplication) source).getAsset());
    ...
    if (ContextScopeHandlerHelper.getInstance(context).contains(ITransitionConstants.SOURCE_SCOPE, source, context)) {
      ContextScopeHandlerHelper.getInstance(context).add(ITransitionConstants.SOURCE_SCOPE,
          ((ThreatApplication) source).getAsset(), context);
    }
  }
}
