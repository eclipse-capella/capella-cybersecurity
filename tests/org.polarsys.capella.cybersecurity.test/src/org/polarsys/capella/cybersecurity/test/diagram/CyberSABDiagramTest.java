...
public class CyberSABDiagramTest extends EmptyProject {
  ...
  public void test() throws Exception {
    ...
    String a1 = diagram.createActor("actor1", diagram.getDiagramId());
    String a2 = diagram.createActor("actor2", diagram.getDiagramId());

    String f1 = diagram.createFunction("f1", a1);
    String f2 = diagram.createFunction("f2", a2);
    ...
    executeCommand(() -> {
      ...
      ((SystemFunction) diagram.getSemanticObjectMap().get("f1")).getOwnedExtensions().add(f1Storage);

      FunctionStorage f2Storage = CybersecurityFactory.eINSTANCE.createFunctionStorage();
      f2Storage.getExchangedItems().add(ei1);
      ((SystemFunction) diagram.getSemanticObjectMap().get("f2")).getOwnedExtensions().add(f2Storage);
    ...
    executeCommand(() -> {
      ...
      InformationPrimaryAsset ipa1 = services.createInformationPrimaryAsset(context.getSemanticElement(getSystemId()));
      ...
      InformationPrimaryAsset ipa2 = services.createInformationPrimaryAsset(context.getSemanticElement(getSystemId()));
    ...
    // scenario 1
    // create 4 actors, each within the previous one
    String actor1 = diagram.createActor("actor1", diagram.getDiagramId());
    String actor2 = diagram.createActor("actor2", actor1);
    String actor3 = diagram.createActor("actor3", actor2);
    String actor4 = diagram.createActor("actor4", actor3);

    // create SystemFunction inside actor4
    String systemFunction1 = diagram.createFunction("systemFunction1", actor4);

    executeCommand(() -> {
      // create FunctionalPrimaryAsset and add SystemFunction1 as member
      fpa = services.createFunctionalPrimaryAsset(context.getSemanticElement(getSystemId()));
      PrimaryAssetMember m1 = CybersecurityFactory.eINSTANCE.createPrimaryAssetMember();
      m1.setMember((ModelElement) diagram.getSemanticObjectMap().get("systemFunction1"));
      fpa.getOwnedMembers().add(m1);
    ...
    DNode fNode = (DNode) diagram.getViewObjectMap().get("systemFunction1");
    assertEquals(fpaColor, ((Square) fNode.getStyle()).getBorderColor());

    RGBValues blackColor = RGBValues.create(0, 0, 0);
    // check that the first actor has the same border color/size as primary asset
    DNodeContainer fNodeContainer = (DNodeContainer) diagram.getViewObjectMap().get("actor4");
    ...
    // check that all other actors have black border color and are not highlighted
    fNodeContainer = (DNodeContainer) diagram.getViewObjectMap().get("actor1");
    ...
    fNodeContainer = (DNodeContainer) diagram.getViewObjectMap().get("actor2");
    assertEquals(blackColor, ((FlatContainerStyle) fNodeContainer.getStyle()).getBorderColor());
    assertEquals(borderSize, ((FlatContainerStyle) fNodeContainer.getStyle()).getBorderSize());

    fNodeContainer = (DNodeContainer) diagram.getViewObjectMap().get("actor3");
    assertEquals(blackColor, ((FlatContainerStyle) fNodeContainer.getStyle()).getBorderColor());
    assertEquals(borderSize, ((FlatContainerStyle) fNodeContainer.getStyle()).getBorderSize());
    ...
    fNode = (DNode) diagram.getViewObjectMap().get("systemFunction1");
    ...
    // check that the first actor has thick border and black color
    ...
    // check that the first actor has thick border and same border color as the threat color
    fNodeContainer = (DNodeContainer) diagram.getViewObjectMap().get("actor4");
    ...
    // create 4 more actors one within the other with a logical function inside the last one
    String actor5 = diagram.createActor("actor5", actor1);
    String actor6 = diagram.createActor("actor6", actor5);
    String actor7 = diagram.createActor("actor7", actor6);
    String actor8 = diagram.createActor("actor8", actor7);

    // create SystemFunction inside actor8
    String systemFunction3 = diagram.createFunction("systemFunction3", actor8);
    executeCommand(() -> {
      ...
      m4.setMember((ModelElement) diagram.getSemanticObjectMap().get("systemFunction3"));
    ...
    // create a component exchange between the 'container' actors
    diagram.createComponentExchange(actor3, actor7, "CE1");
    ...
    executeCommand(() -> {
      ...
      // add exchange item to functional exchange and information primary assets
      diagram.createFunctionalExchange(systemFunction1, systemFunction3, "functionalExchange1");
      FunctionalExchange functionalExchange = (FunctionalExchange) diagram.getSemanticObjectMap()
      ...
      diagram.createPhysicalLink(actor2, actor1, "PL1");
    ...
    // check physical link and component exchange has same color as threat color
